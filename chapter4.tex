\chapter{Parameterized algorithm for pedigree-based phasing}
...
\paragraph{General Approaches for Haplotyping.}
...
The input to the MEC problem consists of a SNP matrix $\mathcal{F}\in\{0,1,-\}^{R\times M}$, where $R$ is the number of reads and $M$ is the number of variants along a chromosome.
...
\begin{figure}%
\includegraphics[width=\columnwidth,height=\columnwidth,keepaspectratio]{{Marschall.152.fig.2a}.pdf}\\
\includegraphics[width=\columnwidth,height=\columnwidth,keepaspectratio]{{Marschall.152.fig.2b}.pdf}%
...
Figure~\ref{fig:reason} illustrates the increased connectivity while phasing a trio, resulting in more phased variants in practice.
