...
The aim of this integrative strategy to obtain dense and global haplotypes that span centromeres, homozygosity regions and genome assembly gaps, while keeping error rates, costs and labor at minimum. 
...
The first ILP formulation for MEC was given by \cite{FM12_solving} and is based on a reduction to the maximum bipartite induced subgraph problem. 
...
Furthermore, there are some approaches to solve other objective functions such as MFR, MSR introduced in Chapter 1.
...
Finding optimal haplotypes is similar to finding optimal bipartition of reads. Once we reach the last column and know the optimal bipartition, we can backtrace to get the optimal haplotypes.
...
\subsubsection{Downsampling of sequencing datasets}
	...
	To achieve this, we downsampled the  original publicly available datasets consisting of: 134 single cell Strand-seq libraries \citep{porubsky2016direct}, 
...
number of switch errors (black crosses) between the true and predicted haplotypes, neighbouring pairs of SNVs
...
\section{Discussion}
	...
	In the next chapters, we will see how we can perform employ trio information instead of Strand-Seq data or performing de novo assembly of haplotype resolved-genomes without the alignment to a reference genome. 
	This will provide us with true diploid representations of individual genomes, which will have profound implications to study variability of personal genomes in health and disease.

\begin{figure}[t!]\centering
\includegraphics[width=\columnwidth]{{Figure1}.pdf}
\caption{Phasing efficacy of read-based and experimental phasing approaches using Chromosome 1 as an (example Chromosome 1).
...
d) Accuracy of each independent phasing approach measured as percentage short switch errors in comparison to benchmark haplotypes.}
\label{fig:fig1}
\end{figure}
...
Sequencing errors (shown in red in read 2 and 7) are corrected when the cost for flipping the alleles is minimized. (b) Reads are then partitioned into two haplotype groups (Haplotpye 1 -- dark blue, Haplotype 2 -- light blue) 
...
% 
\begin{figure}[t!]\centering
\includegraphics[width=\columnwidth, height = .99\columnwidth]{{Figure3}.pdf}
\caption{Various combinations of Strand-seq and read-based phasing (Illumina, PacBio, 10Xusing Genomics) - example Chromosome 1 as an example.
