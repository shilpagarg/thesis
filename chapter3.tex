...
For the classical MEC problem, additionally assuming that all sites to be phased are heterozygous is common \citep{Chen2013}.
...
Let us consider an example to illutrate PedMEC instance shown in Figure~\ref{fig:pedmec4}.
Given a trio with three individuals, mother, father and child. Also, given the input SNP matrices $\mathcal{F}_i$ and its corresponding weight matrices $\mathcal{i}$ for each individual in a trio.
There is a recombination vector $\mathcal{X}$ which represents the likelihood of recombination between two consecutive variants.
Due to the sequencing errors present in the matrices, there are conflicts. The goal is to generate conflict-free matrices such that we obey Mendelian laws of inheritance.
We achieve these objective by jointly minimizing the flipping cost of set of entries over all the individuals and allowing recombination in mother, father or both for every trio relationships.
As a final output, we want two haplotypes for each individual in a trio.
In this example, the cheapest solution is to flip two bits marked in red and, pay cost of 3 and 5, plus allow one recombination event in mother (with cost 22).
In this way, we generate two haplotypes for each individual in a trio with cheapest cost.

Let's see how we can solve this PedMEC instance using a dynamic program. Let us assume there are two haplotypes green and purple for mother, and brown and blue for child shown in Figure~\ref{fig:pedmeccost1}.
...
\caption{Example shows biparition cost for a particular transmission 00 at column one.}
...
\caption{Example shows biparition cost for a particular transmission 00 at column two, given DP column for column one.}
...
\begin{figure}[t]
\begin{center}
\includegraphics[width=.85\columnwidth]{{Marschall.152.fig.3}.pdf}
\end{center}
