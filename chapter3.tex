...
Consider a trio with three individuals, mother, father and child. As input, given SNP matrices $\mathcal{F}_i$ and their corresponding weight matrices $\mathcal{i}$ for an each individual of a trio.
Additionally, there is a recombination vector $\mathcal{X}$ given, representing the likelihood of recombination between two consecutive variants.
The sequencing errors present in reads create conflicts in matrices. The goal is to generate the conflict-free matrices such that we obey Mendelian laws of inheritance.
We can achieve this objective by jointly minimizing the flipping cost of set of entries over all the individuals and, additionally for every trio relationships, allowing recombination in mother, father or both.
As an output, we obtain two haplotypes for an each individual in a trio.

In this example, it is easy to see that the entries marked in in red boxes create conflicts in matrices. 
Therefore, the cheapest solution is to flip these bits by paying a cost of 3 and 5, plus allow one recombination event in mother (with cost 22).
In this way, we generate two haplotypes for an each individual in a trio.

Let us see on how we can solve this PedMEC instance using a dynamic programming based approach. 
Let us assume there are two haplotypes masked in green and purple for mother, and brown and blue for child shown in Figure~\ref{fig:pedmeccost1}.
...
Accordingly, the partitions change and their corresponding allele assignment costs. For the green partition, which now does not span across individuals, cost 0 if flip bits to 0 and otherwise cost 22.
